...
/**
 * Whether the display string is a complete, valid time for the current mode.
 */
export function isCompleteTimeDisplay(
  display: string,
  showSeconds = false,
  hourFormat: TimeHourFormat = '24h'
): boolean {
  return parseDisplay(display, showSeconds, hourFormat) !== null;
}

function mask12h(raw: string, showSeconds: boolean): string {
  const upper = raw.toUpperCase();
  let period = '';
  if (upper.includes('AM') || /(?:^|[^A-Z])A(?:M)?(?:\s|$)/.test(upper)) {
    if (upper.includes('PM')) {
      const amIdx = upper.lastIndexOf('AM');
      const pmIdx = upper.lastIndexOf('PM');
      const aIdx = upper.lastIndexOf('A');
      const pIdx = upper.lastIndexOf('P');
      const lastAm = Math.max(amIdx, aIdx);
      const lastPm = Math.max(pmIdx, pIdx);
      period = lastPm > lastAm ? 'PM' : 'AM';
    } else if (upper.includes('P') && !upper.includes('AM')) {
      period = 'PM';
    } else {
      period = 'AM';
    }
  } else if (upper.includes('PM') || upper.includes('P')) {
    period = 'PM';
  }

  const maxDigits = showSeconds ? 6 : 4;
  const digits = upper.replace(/\D/g, '').slice(0, maxDigits);
  if (!digits && !period) {
    return '';
  }

  let time = digits.slice(0, Math.min(2, digits.length));
  if (digits.length > 2) {
    time += `:${digits.slice(2, Math.min(4, digits.length))}`;
  }
  if (showSeconds && digits.length > 4) {
    time += `:${digits.slice(4, Math.min(6, digits.length))}`;
  }
  return period ? `${time} ${period}` : time;
}

function mask24h(raw: string, showSeconds: boolean): string {
  const maxDigits = showSeconds ? 6 : 4;
...
/**
 * Strip invalid characters and progressively format toward the active display syntax.
 */
export function maskTimeInput(
  raw: string,
  showSeconds = false,
  hourFormat: TimeHourFormat = '24h'
): string {
  return is12HourFormat(hourFormat)
    ? mask12h(raw, showSeconds)
    : mask24h(raw, showSeconds);
}

/** Allowed single characters for beforeinput filtering. */
export function isAllowedTimeChar(
  char: string,
  hourFormat: TimeHourFormat = '24h'
): boolean {
  if (is12HourFormat(hourFormat)) {
    return /^[0-9:APMapm\s]$/.test(char);
  }
