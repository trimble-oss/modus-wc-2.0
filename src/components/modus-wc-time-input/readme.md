...
| `pickerType`      | `picker-type`      | The type of custom picker to render. When undefined (default), the native browser time picker is used. - `'picker'`: Renders a popper panel with side-by-side scrollable columns for hours, minutes, seconds, and AM/PM. - `'datalist'`: Renders a popper panel with a flat list sourced from `datalistOptions`. | `"datalist" \| "picker" \| undefined` | `undefined` |
...
| `use12Hour`       | `use12-hour`       | Display time in 12-hour (AM/PM) format in the picker columns. Only applies when `pickerType` is `'picker'`.                                                                                                                                                                                                      | `boolean \| undefined`                | `false`     |
