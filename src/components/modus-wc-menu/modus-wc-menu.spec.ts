...
describe('modus-wc-menu', () => {
  ...
  it('should not emit menuFocusout when focus moves from expanded parent item to sibling item', async () => {
    const page = await newSpecPage({
      ...
      html: `
        <modus-wc-menu aria-label="Test menu">
          ...
          <modus-wc-menu-item label="Sibling" value="sibling"></modus-wc-menu-item>
        </modus-wc-menu>
      `,
    });
    ...
    const parentItem = page.doc.querySelector(
      'modus-wc-menu-item[value="parent"]'
    ) as HTMLElement;
    const siblingItem = page.doc.querySelector(
      'modus-wc-menu-item[value="sibling"]'
    ) as HTMLElement;

    (parentItem.querySelector('button') as HTMLButtonElement)?.click();
    await page.waitForChanges();
