...
Modus Web Components supports server-side rendering via Stencil's hydrate app.

### Installation

The `hydrate` subpath is included in the package — no additional install needed.
...
**1. Create a reusable `ModusSSR` helper (one-time setup)**
...
**3. Register custom elements for client-side interactivity**

SSR renders the initial HTML, but event handlers require the Stencil runtime on the client.
...
The Modus CSS references font files via relative paths, so it needs to be served statically rather than bundled. Add a `postinstall` script to your `package.json` to automate this:
...
```bash
cp node_modules/@trimble-oss/moduswebcomponents/modus-wc-styles.css public/
...
- `renderToString` runs on the server only — no browser APIs involved.
- The `inputHtml` passed to `ModusSSR` must be trusted markup, not user-supplied content.
- The function is framework-agnostic — works with Next.js, Remix, Express, or any Node.js server.
