...
Please refer to the [official StencilJS documentation](https://stenciljs.com/docs/angular#consumer-usage) for more information on how to integrate with your Angular project.
...
Import Modus Angular Web Components library into your Angular app's module:
...
You can now use Modus Angular Web Components while leveraging Angular template binding syntax:
```ts
// app.component.html
<modus-wc-button label="modus button" />
```
...
import { ModusAngularComponentsModule } from 'component-library';
...
## Consuming Modus Web Components Directly Instead of Using Modus Angular Components

### Installation

Install the Modus Web Component library:

```bash
npm install @trimble-cms/modus-wc
```

Import and execute `defineCustomElements` in your application's main file (e.g., `main.ts`).

If you're unsure about which file is your main file, you can check your application's `angular.json` under `projects`>`architect`>`build`>`options`>`main`
