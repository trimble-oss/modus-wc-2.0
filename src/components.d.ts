/* eslint-disable */
/* tslint:disable */
/**
 * This is an autogenerated file created by the Stencil compiler.
 * It contains typing information for all components that exist in this project.
 */
import { HTMLStencilElement, JSXBase } from '@stencil/core/internal';
export namespace Components {
  interface ModusWcButton {
    ariaLabel: string;
    customClass: string;
    label: string;
  }
}
declare global {
  interface HTMLModusWcButtonElement
    extends Components.ModusWcButton,
      HTMLStencilElement {}
  var HTMLModusWcButtonElement: {
    prototype: HTMLModusWcButtonElement;
    new (): HTMLModusWcButtonElement;
  };
  interface HTMLElementTagNameMap {
    'modus-wc-button': HTMLModusWcButtonElement;
  }
}
declare namespace LocalJSX {
  interface ModusWcButton {
    ariaLabel?: string;
    customClass?: string;
    label?: string;
  }
  interface IntrinsicElements {
    'modus-wc-button': ModusWcButton;
  }
}
export { LocalJSX as JSX };
declare module '@stencil/core' {
  export namespace JSX {
    interface IntrinsicElements {
      'modus-wc-button': LocalJSX.ModusWcButton &
        JSXBase.HTMLAttributes<HTMLModusWcButtonElement>;
    }
  }
}
