...
`}}},render:e=>{const n=(c,o)=>{requestAnimationFrame(()=>{requestAnimationFrame(()=>{const s=Array.from(c.querySelectorAll("modus-wc-tree-item")).find(r=>r.value===o);s&&(s.inlineLabelEdit=!0)})})},l=c=>{var s,r;const o=(s=c.currentTarget)==null?void 0:s.closest("modus-wc-content-tree");if(!o)return;const m=`new-item-${Date.now()}`;(r=o.querySelector(".modus-wc-content-tree-empty-story"))==null||r.remove(),o.items=[{id:m,label:"New Item"}],n(o,m)};return a`
      <style>
        .modus-wc-content-tree-empty-story {
          align-items: center;
          display: flex;
...
<\/script>
`}}},render:e=>{const n=s=>[{id:"toggle-visibility",label:s?"Hidden":"Visible",icon:s?"visibility_off":"visibility_on",ariaLabel:s?"Set item to visible":"Set item to hidden",size:"sm"},{id:"edit-label",label:"Edit Label",icon:"pencil",ariaLabel:"Edit item label",size:"sm"},{id:"duplicate",label:"Duplicate",icon:"copy_content",ariaLabel:"Duplicate item",size:"sm"},{id:"add-node-above",label:"Add Node Above",icon:"add",ariaLabel:"Add node above",size:"sm"},{id:"add-node-below",label:"Add Node Below",icon:"add",ariaLabel:"Add node below",size:"sm"},{id:"add-child",label:"Add Child",icon:"subdirectory_arrow_right",ariaLabel:"Add child node",size:"sm"},{id:"delete",label:"Delete",icon:"delete",ariaLabel:"Delete item",size:"sm"}],l=s=>{s.inlineLabelEdit=!1,s.treeItemActions=n(!!s.disabled),s.querySelectorAll("modus-wc-tree-item").forEach(r=>{const t=r;t.inlineLabelEdit=!1,t.treeItemActions=n(!!t.disabled)})},c=(s="New Node")=>{const r=document.createElement("modus-wc-tree-item");return r.label=s,r.value=`new-node-${Date.now()}-${Math.random()}`,l(r),r};let o=null;const m=s=>{const t=s.target.closest("modus-wc-tree-item");if(!t)return;const i=`${s.detail.actionId}:${t.value}:${s.timeStamp}`;if(o!==i)switch(o=i,queueMicrotask(()=>{o===i&&(o=null)}),s.detail.actionId){case"delete":t.remove();break;case"duplicate":{const d=c(t.label);t.after(d);return}case"add-node-above":t.before(c());break;case"add-node-below":t.after(c());break;case"add-child":{let d=t.querySelector(":scope > modus-wc-tree-view");d||(d=document.createElement("modus-wc-tree-view"),d.setAttribute("is-sub-list","true"),t.appendChild(d)),t.hasSubtree=!0,d.appendChild(c("New Child"));break}case"toggle-visibility":t.disabled=!t.disabled,t.treeItemActions=n(!!t.disabled);break;case"edit-label":t.inlineLabelEdit=!0;break}};return a`
      <modus-wc-content-tree
...
`}}},render:e=>{const n={items:[...e.items??le]},l=c=>{n.items=[...c.detail.items];const o=c.currentTarget;o.items=n.items};return a`
      <modus-wc-content-tree
        search-placeholder=${e["search-placeholder"]}
        customClass=${e["custom-class"]}
        .includeSearch=${e["include-search"]}
...
`}},actions:{handles:["itemExpand"]}},render:e=>{const n=[{id:"documents",label:"Documents",hasChildren:!0},{id:"projects",label:"Projects",hasChildren:!0},{id:"resources",label:"Resources",hasChildren:!0}],l={documents:[{id:"report",label:"Report.pdf"},{id:"proposal",label:"Proposal.docx"},{id:"notes",label:"Meeting Notes.txt"}],projects:[{id:"website",label:"Website Redesign",hasChildren:!0},{id:"mobile",label:"Mobile App"},{id:"api",label:"API Integration"}],resources:[{id:"templates",label:"Templates"},{id:"guide",label:"Style Guide"}],website:[{id:"design",label:"Design Mockups"},{id:"dev",label:"Development"}]},c=(s,r,t)=>s.map(i=>i.id===r?t(i):i.children?{...i,children:c(i.children,r,t)}:i),o=(s,r)=>{for(const t of s){if(t.id===r)return t;if(t.children){const i=o(t.children,r);if(i)return i}}},m=async s=>{var k;const r=s.currentTarget,t=s.detail,i=r.items??n,d=o(i,t);if((k=d==null?void 0:d.children)!=null&&k.length)return;const A=await new Promise(x=>setTimeout(()=>x(l[t]??[]),1500));r.items=c(i,t,x=>({...x,children:A,hasChildren:A.length>0}))};return a`
      <modus-wc-content-tree
        search-placeholder=${e["search-placeholder"]}
        .includeSearch=${e["include-search"]}
        .includeActions=${!1}
