...
<modus-wc-autocomplete
  ...
  value=${e.value}
></modus-wc-autocomplete>
    `},g={...te},y={render:e=>((!e.items||e.items.length===0)&&(e.items=[...f]),e.items=f.map(m=>m.value==="apple"||m.value==="banana"?{...m,selected:!0}:m),v(A||(A=E([`
<script>
...
<modus-wc-autocomplete
  ...
  @inputChange=`,`
></modus-wc-autocomplete>
    `])),e.bordered,n(e["custom-class"]),n(e["debounce-ms"]),e.disabled,n(e["input-id"]),n(e["input-tab-index"]),e.items,n(e.label),e["leave-menu-open"],e["min-chars"],!1,n(e.name),n(e.placeholder),e["read-only"],e.required,e["show-menu-on-focus"],e["show-spinner"],n(e.size),e.value,b)}},x={render:e=>{const m=e["no-results"];e["leave-menu-open"]==!0&&(e["no-results"]={ariaLabel:"",label:"",subLabel:""});const b=l=>{const o=l.querySelectorAll("modus-wc-menu-item:not([disabled])");return Array.from(o).filter(a=>window.getComputedStyle(a).display!=="none"&&!a.classList.contains("hidden"))},h=l=>{var p,d;const o=l.target.closest("modus-wc-autocomplete");if(!o||o.disabled||o.readOnly)return;const t=b(o).map(c=>c.querySelector("button")).filter(Boolean),s=document.activeElement,r=t.indexOf(s);switch(l.key){case"ArrowDown":{l.preventDefault(),o.openMenu();let c=r+1;if(c>=t.length)return;c<0&&(c=0),(p=t[c])==null||p.focus();break}case"ArrowUp":{l.preventDefault(),o.openMenu();let c=r-1;if(c<0)return;(d=t[c])==null||d.focus();break}case"Enter":{l.preventDefault(),t.includes(s)&&s.click();const c=o.querySelector("input");c==null||c.focus();break}case"Escape":{l.preventDefault(),o.closeMenu();const c=o.querySelector("input");c==null||c.focus();break}}},u=l=>{var a;if(!((a=l.detail)!=null&&a.target))return;const o=l.target.closest("modus-wc-autocomplete");if(o){const t=l.detail.target.value.toLowerCase(),s=o==null?void 0:o.querySelectorAll("modus-wc-menu-item");t===""&&(s==null||s.forEach(d=>{d.removeAttribute("selected")}));let r=0;Array.from(s??[]).forEach(d=>{var w;(((w=d.getAttribute("label"))==null?void 0:w.toLowerCase())||"").includes(t)?d.classList.remove("hidden"):(d.classList.add("hidden"),r++)}),o.noResults=r===(s==null?void 0:s.length)?m:{ariaLabel:"",label:"",subLabel:""};const p=o.querySelector(".no-results-item");p&&(r===(s==null?void 0:s.length)?p.classList.add("visible"):p.classList.remove("visible"))}},i=l=>{const o=l.target.closest("modus-wc-autocomplete");if(o){const a=l.detail.value;o.value=a,o.querySelectorAll("modus-wc-menu-item").forEach(s=>{s.getAttribute("value")===a?s.setAttribute("selected","true"):s.removeAttribute("selected")}),e["leave-menu-open"]||o.closeMenu()}};return v(L||(L=E([`
<script>
...
>
  <div slot="menu-items">
    ...
    `])),e.bordered,n(e["custom-class"]),n(e["debounce-ms"]),e.disabled,n(e["input-id"]),n(e["input-tab-index"]),n(e.label),e["leave-menu-open"],e["min-chars"],!1,n(e.name),e["no-results"],n(e.placeholder),e["read-only"],e.required,e["show-menu-on-focus"],n(e.size),e["show-spinner"],e.value,h,u,!0,i,i,i,i)}},M={render:e=>{const m=u=>{const i=u.target.closest("modus-wc-autocomplete");if(!i)return;["ArrowDown","ArrowUp","Enter","Escape"].includes(u.key)&&u.preventDefault();const l=e.items.filter(o=>o.visibleInMenu&&!o.disabled);switch(u.key){case"Escape":e.items=e.items.map(o=>({...o,focused:!1})),i.items=[...e.items],i.closeMenu(),i.style.transform="scale(0.98)",setTimeout(()=>{i.style.transform=""},200);break;case"ArrowDown":{i.openMenu();const o=l.findIndex(s=>s.focused),a=o<0?0:Math.min(o+1,l.length-1),t=a+1<l.length?a+1:a;e.items=e.items.map(s=>{var r;return{...s,focused:((r=l[t])==null?void 0:r.value)===s.value}});break}case"ArrowUp":{const o=l.findIndex(s=>s.focused),a=o<0?l.length-1:Math.max(o-1,0),t=a-1>=0?a-1:a;e.items=e.items.map(s=>{var r;return{...s,focused:((r=l[t])==null?void 0:r.value)===s.value}});break}case"Enter":{const o=l.find(a=>a.focused);o&&(e.items=e.items.map(a=>({...a,selected:a.value===o.value,focused:!1})),i.value=o.label,i.closeMenu());break}default:return}i.items=[...e.items]},b=u=>{const i=document.querySelector("modus-wc-autocomplete");if(!i)return;const l=u.toLowerCase().split("");if(u.length>0){const t=e.items.map(s=>{const r=s.label.toLowerCase();let p=0,d=!0;for(const c of l)if(r.includes(c)){const w=r.indexOf(c);w===0?p+=3:r[w-1]===" "?p+=2:p+=1}else{d=!1;break}return d&&r.includes(u.toLowerCase())&&(p+=10),{item:s,score:d?p:-1,visible:d}});t.sort((s,r)=>r.score-s.score),e.items=t.map(({item:s,visible:r})=>({...s,visibleInMenu:r,focused:!1,selected:s.selected&&r,label:s.label}))}else e.items=e.items.map(t=>({...t,visibleInMenu:!0,focused:!1}));i.items=[...e.items],i.value=u;const o=e.items.filter(t=>t.visibleInMenu).length;console.log(`Fuzzy search for "${u}": ${o} matches found`),e.items.some(t=>t.visibleInMenu)&&u.length>=e["min-chars"]?i.openMenu():i.closeMenu()},h=u=>{const i=document.querySelector("modus-wc-autocomplete");i&&(e.items=e.items.map(l=>({...l,selected:l.value===u.value,focused:!1})),i.items=[...e.items],i.value=u.label,i.closeMenu())};return v`
      <style>
...
Items are automatically sorted by relevance with exact substring matches appearing first.`}}}},$={args:{...ee.args,items:f},render:e=>{const m=async()=>{const t=document.getElementById("programmatic-autocomplete");if(t){const s=f.find(r=>r.value==="apple")||null;await t.selectItem(s)}},b=async()=>{const t=document.getElementById("programmatic-autocomplete");t&&await t.selectItem(null)},h=async()=>{const t=document.getElementById("programmatic-autocomplete");t&&await t.openMenu()},u=async()=>{const t=document.getElementById("programmatic-autocomplete");t&&await t.closeMenu()},i=async()=>{const t=document.getElementById("programmatic-autocomplete");t&&await t.toggleMenu()},l=async()=>{const t=document.getElementById("programmatic-autocomplete");t&&await t.focusInput()},o=async()=>{const t=document.getElementById("programmatic-autocomplete");t&&await t.clearInput()},a=window;return a.handleSelectApple=m,a.handleSelectNull=b,a.handleOpenMenu=h,a.handleCloseMenu=u,a.handleToggleMenu=i,a.handleFocusInput=l,a.handleClearInput=o,v`
      <style>
...
\`\`\`
        `}},controls:{disable:!0},canvas:{disable:!0}},render:()=>v`<div></div>`};var k,z,D;g.parameters={...g.parameters,docs:{...(k=g.parameters)==null?void 0:k.docs,source:{originalSource:`{
  ...Template
...
}`,...(H=(q=y.parameters)==null?void 0:q.docs)==null?void 0:H.source}}};var B,R,F;I.parameters={...I.parameters,docs:{...(B=I.parameters)==null?void 0:B.docs,source:{originalSource:`{
  render: args => {
    let debounceTimer: number;
    const handleInputChange = (e: CustomEvent<Event>) => {
      if (!e.detail?.target) return;
...
}`,...(F=(R=I.parameters)==null?void 0:R.docs)==null?void 0:F.source}}};var O,N,j;x.parameters={...x.parameters,docs:{...(O=x.parameters)==null?void 0:O.docs,source:{originalSource:`{
  render: args => {
    const originalNoResults = args['no-results'];
    if (args['leave-menu-open'] == true) {
      args['no-results'] = {
