...
>
  <div slot="menu-items">
    ...
    `])),e.bordered,n(e["custom-class"]),n(e["debounce-ms"]),e.disabled,n(e["input-id"]),n(e["input-tab-index"]),n(e.label),e["leave-menu-open"],e["min-chars"],!1,n(e.name),e["no-results"],n(e.placeholder),e["read-only"],e.required,e["show-menu-on-focus"],n(e.size),e["show-spinner"],e.value,b,c,!0,s,s,s,s)}},S={render:e=>{const m=c=>{const s=document.getElementById("custom-event-handlers");if(!s)return;["ArrowDown","ArrowUp","Enter","Escape"].includes(c.key)&&c.preventDefault();const a=e.items.filter(t=>t.visibleInMenu&&!t.disabled);switch(c.key){case"Escape":e.items=e.items.map(t=>({...t,focused:!1})),s.items=[...e.items],s.closeMenu(),s.style.transform="scale(0.98)",setTimeout(()=>{s.style.transform=""},200);break;case"ArrowDown":{s.openMenu();const t=a.findIndex(o=>o.focused),i=t<0?0:Math.min(t+1,a.length-1),l=i+1<a.length?i+1:i;e.items=e.items.map(o=>{var u;return{...o,focused:((u=a[l])==null?void 0:u.value)===o.value}});break}case"ArrowUp":{const t=a.findIndex(o=>o.focused),i=t<0?a.length-1:Math.max(t-1,0),l=i-1>=0?i-1:i;e.items=e.items.map(o=>{var u;return{...o,focused:((u=a[l])==null?void 0:u.value)===o.value}});break}case"Enter":{const t=a.find(i=>i.focused);t&&(e.items=e.items.map(i=>({...i,selected:i.value===t.value,focused:!1})),s.value=t.label,s.closeMenu());break}default:return}s.items=[...e.items]},h=c=>{const s=document.getElementById("custom-event-handlers");if(!s)return;const a=c.toLowerCase().split("");if(c.length>0){const l=e.items.map(o=>{const u=o.label.toLowerCase();let p=0,d=!0;for(const r of a)if(u.includes(r)){const w=u.indexOf(r);w===0?p+=3:u[w-1]===" "?p+=2:p+=1}else{d=!1;break}return d&&u.includes(c.toLowerCase())&&(p+=10),{item:o,score:d?p:-1,visible:d}});l.sort((o,u)=>u.score-o.score),e.items=l.map(({item:o,visible:u})=>({...o,visibleInMenu:u,focused:!1,selected:o.selected&&u,label:o.label}))}else e.items=e.items.map(l=>({...l,visibleInMenu:!0,focused:!1}));s.items=[...e.items],s.value=c;const t=e.items.filter(l=>l.visibleInMenu).length;console.log(`Fuzzy search for "${c}": ${t} matches found`),e.items.some(l=>l.visibleInMenu)&&c.length>=e["min-chars"]?s.openMenu():s.closeMenu()},b=c=>{const s=document.getElementById("custom-event-handlers");s&&(e.items=e.items.map(a=>({...a,selected:a.value===c.value,focused:!1})),s.items=[...e.items],s.value=c.label,s.closeMenu())};return f(F||(F=v([`
      <style>
...
>
  ...
  render: args => {
    ...
    const customKeyDown = (e: KeyboardEvent) => {
      const autocomplete = document.getElementById('custom-event-handlers') as AutocompleteElement;
      if (!autocomplete) return;
