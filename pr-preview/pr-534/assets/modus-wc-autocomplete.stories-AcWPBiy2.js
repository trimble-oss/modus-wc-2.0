...
>
  <div slot="menu-items">
    ...
    `])),e.bordered,n(e["custom-class"]),n(e["debounce-ms"]),e.disabled,n(e["input-id"]),n(e["input-tab-index"]),n(e.label),e["leave-menu-open"],e["min-chars"],!1,n(e.name),e["no-results"],n(e.placeholder),e["read-only"],e.required,e["show-menu-on-focus"],n(e.size),e["show-spinner"],e.value,h,u,!0,i,i,i,i)}},M={render:e=>{const p=u=>{const i=u.target.closest("modus-wc-autocomplete");if(!i)return;["ArrowDown","ArrowUp","Enter","Escape"].includes(u.key)&&u.preventDefault();const l=e.items.filter(o=>o.visibleInMenu&&!o.disabled);switch(u.key){case"Escape":e.items=e.items.map(o=>({...o,focused:!1})),i.items=[...e.items],i.closeMenu(),i.style.transform="scale(0.98)",setTimeout(()=>{i.style.transform=""},200);break;case"ArrowDown":{i.openMenu();const o=l.findIndex(s=>s.focused),a=o<0?0:Math.min(o+1,l.length-1),t=a+1<l.length?a+1:a;e.items=e.items.map(s=>{var r;return{...s,focused:((r=l[t])==null?void 0:r.value)===s.value}});break}case"ArrowUp":{const o=l.findIndex(s=>s.focused),a=o<0?l.length-1:Math.max(o-1,0),t=a-1>=0?a-1:a;e.items=e.items.map(s=>{var r;return{...s,focused:((r=l[t])==null?void 0:r.value)===s.value}});break}case"Enter":{const o=l.find(a=>a.focused);o&&(e.items=e.items.map(a=>({...a,selected:a.value===o.value,focused:!1})),i.value=o.label,i.closeMenu());break}default:return}i.items=[...e.items]},b=u=>{const i=document.querySelector("modus-wc-autocomplete");if(!i)return;const l=u.toLowerCase().split("");if(u.length>0){const t=e.items.map(s=>{const r=s.label.toLowerCase();let d=0,m=!0;for(const c of l)if(r.includes(c)){const w=r.indexOf(c);w===0?d+=3:r[w-1]===" "?d+=2:d+=1}else{m=!1;break}return m&&r.includes(u.toLowerCase())&&(d+=10),{item:s,score:m?d:-1,visible:m}});t.sort((s,r)=>r.score-s.score),e.items=t.map(({item:s,visible:r})=>({...s,visibleInMenu:r,focused:!1,selected:s.selected&&r,label:s.label}))}else e.items=e.items.map(t=>({...t,visibleInMenu:!0,focused:!1}));i.items=[...e.items],i.value=u;const o=e.items.filter(t=>t.visibleInMenu).length;console.log(`Fuzzy search for "${u}": ${o} matches found`),e.items.some(t=>t.visibleInMenu)&&u.length>=e["min-chars"]?i.openMenu():i.closeMenu()},h=u=>{const i=document.querySelector("modus-wc-autocomplete");i&&(e.items=e.items.map(l=>({...l,selected:l.value===u.value,focused:!1})),i.items=[...e.items],i.value=u.label,i.closeMenu())};return f`
      <style>
