...
`,a=new WeakSet;let t=null;const r=new WeakSet;let d=null;async function b(o,s=!1){if(o instanceof ShadowRoot)if(s){if(r.has(o))return;const c=m+`
`+u+`
`+i+`
`+w;if(!c.trim()){console.warn("No CSS content available for shadow DOM injection");return}if("adoptedStyleSheets"in Document.prototype&&"CSSStyleSheet"in window)try{d||(d=new CSSStyleSheet,await d.replace(c));const n=o;n.adoptedStyleSheets=[...n.adoptedStyleSheets||[],d]}catch(n){console.warn("adoptedStyleSheets failed, falling back to <style> element:",n);const e=document.createElement("style");e.textContent=c,o.appendChild(e)}else{const n=document.createElement("style");n.textContent=c,o.appendChild(n)}r.add(o),a.add(o)}else{if(r.has(o)||a.has(o))return;const c=m+`
`+u+`
`+i;if(!c.trim()){console.warn("No CSS content available for shadow DOM injection");return}if("adoptedStyleSheets"in Document.prototype&&"CSSStyleSheet"in window)try{t||(t=new CSSStyleSheet,await t.replace(c));const n=o;n.adoptedStyleSheets=[...n.adoptedStyleSheets||[],t]}catch(n){console.warn("adoptedStyleSheets failed, falling back to <style> element:",n);const e=document.createElement("style");e.textContent=c,o.appendChild(e)}else{const n=document.createElement("style");n.textContent=c,o.appendChild(n)}a.add(o)}}function h(o,s=!1){const c=o.getRootNode();c instanceof ShadowRoot&&b(c,s)}export{h};
