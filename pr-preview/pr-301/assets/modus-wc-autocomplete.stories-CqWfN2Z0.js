...
<modus-wc-autocomplete
  ...
  @itemSelect=`,`
></modus-wc-autocomplete>
    `])),e.bordered,n(e["custom-class"]),n(e["debounce-ms"]),e.disabled,n(e["input-id"]),n(e["input-tab-index"]),e.items,n(e.label),e["leave-menu-open"],e["min-chars"],!0,n(e.name),e["no-results"],n(e.placeholder),e["read-only"],e.required,e["show-spinner"],n(e.size),e.value,r,c,l)}},f={render:e=>{const r=l=>{var a;if(!((a=l.detail)!=null&&a.target))return;const t=l.target.closest("modus-wc-autocomplete");if(t){const s=l.detail.target,o=s.value.toLowerCase();setTimeout(()=>{t.showSpinner=!1},2e3),t.showSpinner=!0;const m=i.map(u=>({...u,selected:o?u.selected:!1,visibleInMenu:u.label.toLowerCase().includes(o)}));t.items=[...m],t.value=s.value}},c=l=>{const t=l.target.closest("modus-wc-autocomplete");if(t){const a=l.detail.label;a&&(t.value=a),i.forEach(o=>o.selected=!1);const s=i.find(o=>o.value===l.detail.value);s&&(s.selected=!0,t.items=[...i])}};return v(C||(C=b([`
<style>
  /* Only for Storybook */
  div[id^="story--components-forms-autocomplete--default"] {
    height: 400px;
