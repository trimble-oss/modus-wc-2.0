 ...
 * - Storybook examples that nest <child> inside <parent> → "storybook" edges
 *   (demo composition only, e.g. toast stories slot modus-wc-alert).
...
function scanStorybookEdges(knownTags, composeKeys) {
  const edgeFiles = new Map();
  ...
  return toEdgeList(edgeFiles, 'storybook');
}
...
function toMermaid(edges) {
  const lines = [
    ...
    '%% Solid: JSX composition. Dotted: slot/hosts. Thick: Storybook examples.',
    'flowchart LR',
  ];
...
function main() {
  ...
  const hostedEdges = scanHostedEdges(knownTags, composeKeys);
  const storybookEdges = scanStorybookEdges(knownTags, composeKeys);
  ...
  console.log(
    `component-graph: ${graph.nodes.length} nodes, ${edges.length} edges ` +
      `(${composesEdges.length} composes, ${slotEdges.length} slot, ${hostedEdges.length} hosts, ${storybookEdges.length} storybook) → ${relative(ROOT, OUT_DIR)}/`
  );
}
