...
`,s=new WeakSet;let t=null;const r=new WeakSet;let d=null;async function b(o,a=!1){if(o instanceof ShadowRoot)if(a){if(r.has(o))return;const c=m+`
`+u+`
`+i+`
`+w;if(!c.trim()){console.warn("No CSS content available for shadow DOM injection");return}if("adoptedStyleSheets"in Document.prototype&&"CSSStyleSheet"in window)try{d||(d=new CSSStyleSheet,await d.replace(c));const e=o;e.adoptedStyleSheets=[...e.adoptedStyleSheets||[],d]}catch(e){console.warn("adoptedStyleSheets failed, falling back to <style> element:",e);const n=document.createElement("style");n.textContent=c,o.appendChild(n)}else{const e=document.createElement("style");e.textContent=c,o.appendChild(e)}r.add(o),s.add(o)}else{if(r.has(o)||s.has(o))return;const c=m+`
`+u+`
`+i;if(!c.trim()){console.warn("No CSS content available for shadow DOM injection");return}if("adoptedStyleSheets"in Document.prototype&&"CSSStyleSheet"in window)try{t||(t=new CSSStyleSheet,await t.replace(c));const e=o;e.adoptedStyleSheets=[...e.adoptedStyleSheets||[],t]}catch(e){console.warn("adoptedStyleSheets failed, falling back to <style> element:",e);const n=document.createElement("style");n.textContent=c,o.appendChild(n)}else{const e=document.createElement("style");e.textContent=c,o.appendChild(e)}s.add(o)}}function h(o,a=!1){const c=o.getRootNode();c instanceof ShadowRoot&&b(c,a)}export{h};
