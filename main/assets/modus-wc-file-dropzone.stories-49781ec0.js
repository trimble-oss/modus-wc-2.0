...
<\/script>`}}},render:e=>n`
  ...
  `},c={args:{multiple:!0,"accept-file-types":"image/*"},render:e=>n`
    ...
    ></modus-wc-file-dropzone>
  `},d={render:e=>{if(!customElements.get("file-dropzone-shadow-host")){const i=B({componentTag:"modus-wc-file-dropzone",propsMapper:(o,F)=>{const t=F;t.acceptFileTypes=o["accept-file-types"]??"",t.customClass=o["custom-class"]||"",t.disabled=Boolean(o.disabled),t.fileDraggedOverInstructions=o["file-dragged-over-instructions"]??"",t.includeStateIcon=Boolean(o["include-state-icon"]),t.instructions=o.instructions??"",t.invalidFileTypeMessage=o["invalid-file-type-message"]??"",t.maxFileCount=o["max-file-count"]??0,t.maxFileNameLength=o["max-file-name-length"]??0,t.maxTotalFileSizeBytes=o["max-total-file-size-bytes"]??0,t.multiple=Boolean(o.multiple),t.successMessage=o["success-message"]??""}});customElements.define("file-dropzone-shadow-host",i)}return n`<file-dropzone-shadow-host
      .props=${{...e}}
