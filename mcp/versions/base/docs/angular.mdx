...
> **Angular 17, 18, and 19 notice:** The `@trimble-oss/moduswebcomponents-angular`
...
>
> If you are on Angular 17, 18, or 19 you can use Modus Web Components **directly**
> without the Angular wrapper package. Angular supports custom elements natively via
> `CUSTOM_ELEMENTS_SCHEMA`. See the [Using Web Components Directly](#using-web-components-directly-in-angular) section below.
>
> We recommend upgrading to Angular 20 or later to continue using the full Angular wrapper.

This guide will help you get started with consuming the Modus Angular Web Component library in your Angular project.

We highly recommend using the Modus Angular Components library for Angular based projects on **Angular 20+**.
...
## Angular 20+ with the wrapper package
...
`provideModusWebComponents()`. There is no `ModusAngularComponentsModule`.

### 1. Install both `modus-wc` and `modus-wc-angular` dependencies:

Ensure that you specify the target version of Angular for the `modus-wc-angular` package (e.g., `ng20` for Angular 20).
...
### 4. Provide Modus Web Components at app bootstrap:
...
### 5. Import the standalone proxy components you need:
...
### 6. Use Modus Angular Web Components while leveraging Angular template binding syntax:
...
## Using Web Components Directly in Angular

If you are on Angular 17, 18, or 19 (or prefer not to use the Angular wrapper), you can
consume `@trimble-oss/moduswebcomponents` directly. No `@trimble-oss/moduswebcomponents-angular`
package is required.

### 1. Install only the web components package:

```bash
npm install @trimble-oss/moduswebcomponents
```

### 2. Set up styling, assets, and register custom elements:

Configure Angular to copy Modus static assets (needed for components like `modus-wc-logo`) in `angular.json` under `projects > your-app > architect > build > options`:
...
Then import styles and initialize Modus once during application startup (for example in `main.ts`):
...
### 3. Tell Angular about custom elements:

In an NgModule app, add `CUSTOM_ELEMENTS_SCHEMA` so Angular does not error on unknown element names:
...
In a standalone app, add the schema on the root component (or each component that uses Modus tags):
...
### 4. Use Modus tags with Angular template bindings:

Angular allows `[]` and `()` markup syntax for the web component's inputs and outputs:
